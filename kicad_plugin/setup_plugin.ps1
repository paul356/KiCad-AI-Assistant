...
if (Test-Path $VersionFile) {
    ...
    $Line = Get-Content $PyprojectFile | Select-String -Pattern '^version = "(.+)"' | Select-Object -First 1
    if (-not $Line) {
        Write-Host "ERROR: Could not extract version from $PyprojectFile"
        exit 1
    }
    $KcaaVersion = $Line.Matches.Groups[1].Value
    Write-Host "kcaa version: $KcaaVersion (from pyproject.toml)"
} else {
    Write-Host "ERROR: VERSION file not found at $VersionFile and pyproject.toml not found at $PyprojectFile."
    Write-Host "Run 'make dist-plugin' or place a pyproject.toml next to the setup script."
    exit 1
}
