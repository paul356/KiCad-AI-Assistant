...
2. Call **list_footprints** to get every footprint's reference, value, x/y
   (world mm), rotation (CCW+), and layer.  Optionally filter with
   ``ref_prefix`` (e.g. ``"R"``) when you only need a subset of references.
...
4. Call **list_nets** to enumerate nets with their pad counts
   (``classify=True`` adds netclass/type).  **list_vias** accepts a
   ``net`` filter.
