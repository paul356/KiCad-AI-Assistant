...
- Move or rotate footprints: **set_footprint_position(pcb_path,
  references, x, y, rotation)**.  ``references`` takes one or more
  designators (e.g. ``["U1", "R2"]``).  Any of x/y/rotation may be ``null``
  to leave it unchanged; at least one must be provided.  If a requested
  position causes a courtyard collision, the tool automatically adjusts to
  the nearest free spot; if none is found within 20 mm, that footprint
  keeps a per-reference error while the rest of the batch is still applied.
...
- Update a footprint property (Reference, Value, Datasheet, or custom field)
  on one or more footprints:
  **set_footprint_property(pcb_path, references, property_name, value)**.
