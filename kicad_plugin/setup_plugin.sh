...
if [[ -f "$VERSION_FILE" ]]; then
    ...
    echo "kcaa version: $KCAA_VERSION (from pyproject.toml)"
elif [[ -f "$PYPROJECT_FILE_PARENT" ]]; then
    KCAA_VERSION="$(grep -m1 '^version = ' "$PYPROJECT_FILE_PARENT" | sed 's/version = "\(.*\)"/\1/')"
    if [[ -z "$KCAA_VERSION" ]]; then
        echo "ERROR: Could not extract version from $PYPROJECT_FILE_PARENT"
        exit 1
    fi
    echo "kcaa version: $KCAA_VERSION (from pyproject.toml)"
else
    echo "ERROR: VERSION file not found at $VERSION_FILE and pyproject.toml not found at $PYPROJECT_FILE or $PYPROJECT_FILE_PARENT."
    echo "Run 'make dist-plugin' or place a pyproject.toml next to the setup script."
    exit 1
fi
